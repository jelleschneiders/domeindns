            ...
            <div class="col-md-6">
                <div class="card shadow mb-4">
                    <div class="card-header py-3">
                        <h6 class="m-0 font-weight-bold text-primary">Update record for template {{ $template->name }}</h6>
                    </div>

                    <div class="card-body">
                        @include('errors')

                        <form method="POST" action="/template/{{ $template->id }}/records/{{ $record->id }}/edit">
                            @csrf
                            @method('PATCH')
